...
from constants import num_actions_fine, num_windows_cd, num_windows_fd, num_actions_coarse, img_size_fd
...
def test(epoch):
    ...
    for batch_idx, (inputs_cd, targets) in tqdm.tqdm(enumerate(testloader), total=len(testloader)):
        ...
        for xind in range(num_windows_cd):
            for yind in range(num_windows_cd):
                ...
                # -----------------------------------------------
                policy_fd = torch.zeros((args.batch_size, num_actions_fine))
                ...
                metrics, set_labels = utils.get_detected_boxes(policy_fd, targets_ind, num_windows_fd, base_dir_fd,
                                    base_dir_cd, base_dir_gt, metrics, set_labels) #, zoomed_ins, num_objects)
    ...
    print 'Test - AP: %.3f | AR : %.3f | RS : %.3f'%(AP[0], recall.mean(), num_sampled/num_total)

#--------------------------------------------------------------------------------------------------------#
_, testset = utils.get_dataset(args.model, args.img_size, args.data_dir)
testloader = torchdata.DataLoader(testset, batch_size=args.batch_size, shuffle=False, num_workers=16)
agent_cd = utils.get_model(args.model, num_actions_coarse)
agent_fd = utils.get_model(args.model, num_actions_fine)
